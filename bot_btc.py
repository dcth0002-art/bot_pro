...
class TradingBot:
    def __init__(self):
        ...
        self.current_position = None  # 'buy' hoặc 'sell'
        self.active_symbol = None     # Đồng tiền đang vào lệnh
        self.entry_price = 0
        self.amount_coin = 0
        
        # Cấu trúc dữ liệu riêng cho từng đồng
        self.coins = {}
    ...
    def update_coin_data(self, symbol):
        """Cập nhật Vol và Giá cho một đồng tiền cụ thể."""
        try:
            c = self.coins[symbol]
    ...
    def run(self):
        send_telegram(f"🚀 *Bot Săn Lệnh Đa Coin (Tốc độ cao) đã khởi động!*\n- Danh sách: `{len(SYMBOLS)} đồng`\n- Khoảng nghỉ: `1 giây` giữa các chu kỳ.")
            ...
            if not self.is_warmed_up:
                if current_time - self.start_time >= WARMUP_PERIOD:
                    ...
                    for sym in SYMBOLS:
                        ...
                        time.sleep(0.05) # Giảm nghỉ để quét nhanh hơn
                    print(f"Đang tích lũy dữ liệu... còn {int(WARMUP_PERIOD - (current_time - self.start_time))}s")
                    continue

            # --- TRƯỜNG HỢP 1: ĐANG TRỐNG LỆNH - ĐI SĂN ---
                    ...
                    if current_time - c['last_close_time'] >= COOLDOWN_PERIOD:
                        if c['pending_side'] is None:
                            if buy_diff > VOL_DIFF_THRESHOLD and current_price > price_trend_ago:
                                ...
                                print(f"🔍 [{symbol}] Vol Mua mạnh! Chờ 60s...")
                            elif sell_diff > VOL_DIFF_THRESHOLD and current_price < price_trend_ago:
                                c['pending_side'] = 'sell'
                                c['trigger_price'] = current_price
                                c['trigger_time'] = current_time
                                print(f"🔍 [{symbol}] Vol Bán mạnh! Chờ 60s...")
                        else:
                            ...
                            if c['pending_side'] == 'buy':
                                if current_price < c['trigger_price']:
                                    ...
                                    self.open_position(symbol, 'buy', current_price, buy_diff)
                                    break
                            elif c['pending_side'] == 'sell':
                                if current_price > c['trigger_price']:
                                    c['pending_side'] = None
                                elif elapsed >= CONFIRMATION_TIME:
                                    self.open_position(symbol, 'sell', current_price, sell_diff)
                                    break
                    
                    time.sleep(0.05) # Giảm nghỉ giữa các coin để đạt tốc độ 1s tổng thể

            # --- TRƯỜNG HỢP 2: ĐANG CÓ LỆNH - CHĂM SÓC ---
            else:
    ...
    def open_position(self, symbol, side, price, diff):
        ...
        msg = (
            ...
            f"💰 Giá vào: `{price:,.4f}`\n"
            f"📊 Chênh lệch Vol: `+{diff*100:.1f}%` (Xác nhận 60s xong)\n"
    ...
    def send_multi_report(self):
        msg = "📊 *TRẠNG THÁI HỆ THỐNG*\n"
        if self.active_symbol:
            msg += f"📍 Đang trade: `{self.active_symbol}`\n"
        else:
            msg += "📍 Trạng thái: `Đang săn tìm tín hiệu...`\n"
        msg += f"🏦 Tổng vốn: `${self.balance:,.2f}$`"
        send_telegram(msg)
